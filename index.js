...
const saveFeedback = (phoneNumber, message) => {
  const feedbacks = loadFeedbacks();
  feedbacks.push({ phoneNumber, message, createdAt: new Date() });
...
const loadFeedbacks = () => {
  try {
    const data = fs.readFileSync("feedbacks.json");
    return JSON.parse(data);
  } catch (error) {
    return [];
  }
...
app.post("/ussd", (req, res) => {
  ...
  // Contact Support
    ...
    if (message.length > 160) {
      ...
      saveFeedback(phoneNumber, message);
